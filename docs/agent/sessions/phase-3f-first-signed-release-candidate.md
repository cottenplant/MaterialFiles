...
reviewed source commit. The candidate is Moby Files 1.7.4, version code 39, and
is designated RC1 outside the Android version fields. Build the guarded unsigned
...
- Treat the current Android version name `1.7.4` and version code 39 as fixed
  for RC1. The RC designation belongs in release records and filenames only;
  no source or resource version change is authorized.
- Create two clean detached worktrees, separate build outputs, logs, cloned
  Gradle state, and artifact handoff paths only under `.gradle/phase-3f/`, which
  is already ignored.
- Seed Phase 3F Gradle state only from the retained repository-local Phase 3B
  state. Do not read or populate the default Gradle cache.
...
- Execute the pinned Gradle wrapper, the existing host JDK selected by the
  wrapper, installed Android Build Tools, NDK and CMake, and offline host tools
  needed for byte, ZIP, size, and SHA-256 comparisons.
- Place one unchanged canonical unsigned APK at
  `.gradle/phase-3f/handoff/MobyFiles-1.7.4-unsigned.apk` for the release owner
  to copy into a private release workspace.
- Receive exactly two signed APK copies at
  `.gradle/phase-3f/inbox/MobyFiles-1.7.4-rc1-sign-a.apk` and
  `.gradle/phase-3f/inbox/MobyFiles-1.7.4-rc1-sign-b.apk`. These APKs are
...
- No application, resource, manifest, permission, identity, namespace, version,
  dependency, lockfile, verification metadata, wrapper, SDK, NDK, CMake,
  signing-configuration, CI, or Fastlane change.
...
- The committed source tree is clean, resolves to one full Phase 3F commit, and
  retains version name `1.7.4`, version code 39, application ID
  `io.github.cottenplant.mobyfiles`, and label `Moby Files`.
...
- No artifact, certificate, credential, private record, or ignored Phase 3F
  ...
  contract, an optional separately committed signing-guide safety correction,
...
- SDK-backed work requires explicit command-boundary approval for read-only
  ...
  is authorized.
- Offline assembly depends on the retained repository-local Phase 3B Gradle
  state. A cache miss or integrity failure stops the phase for a new decision.
